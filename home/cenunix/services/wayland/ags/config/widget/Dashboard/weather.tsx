...
const WeatherBoxChild = (w: TooltipItem) => (
  <box
    ...
    children={[
      ...
      <label
        label={"  " + Math.round(Number(w.wind.replace(/mph$/, ""))) + " mph"}
...
const WeatherBoxChildWrapper = (
  w: TooltipItem,
  temperatureDataPerDay: Record<string, TemperatureData>,
  totalWeatherForecastDataArray: TooltipItem[],
) => (
  <box
    ...
    setup={(self) => {
      ...
      if (
        ...
        temperatureDataPerDay[w.date.substring(0, 3).toUpperCase()].data.length
      ) {
        temperatureDataPerDay[
    ...
    children={[
      <box
        ...
        children={[
          ...
          rain != 0 ? (
            <label label={rain + "in"} className={"weather-rain"} />
          ) : (
            <label />
          ),
          <box hexpand={true} />,
          <label
            label={
              "↑ " +
              temperatureDataPerDay[widgetDate.substring(0, 3).toUpperCase()]
                .maxTemp +
              "  "
            }
...
export const Tooltip = ({ total }: { total: number | null }) => (
  <box
    className={namespace}
    setup={(self) => {
      self.hook(weather, () => {
