{ _7zz, buildDotnetModule, copyDesktopItems, desktop-file-utils
, dotnetCorePackages, fetchFromGitHub, fontconfig, lib, libICE, libSM, libX11
, nexus-mods-app, runCommand
...
let
  ...
  meta = {
    ...
    changelog =
      "https://github.com/Nexus-Mods/NexusMods.App/releases/tag/${src.rev}";
...
in buildDotnetModule {
  ...
  postPatch = ''
    ln --force --symbolic "${
      lib.getExe _7zzWithOptionalUnfreeRarSupport
    }" src/ArchiveManagement/NexusMods.FileExtractor/runtimes/linux-x64/native/7zz
  '';
  ...
  runtimeDeps = [ fontconfig libICE libSM libX11 ];

  executables = [ nexus-mods-app.meta.mainProgram ];
