...
let
  x13s-firm = stdenvNoCC.mkDerivation rec {
    pname = "x13s-firmware";
    version = "1.0.0";
  ...
  xz_blacklist = [ x13s-firm ];
in
{
  nixpkgs.overlays = [
    (self: super: {
      compressFirmwareXz = firmware:
        if builtins.elem firmware xz_blacklist
        then firmware
        else super.compressFirmwareXz firmware;
    })
  ];
  hardware.firmware = [ x13s-firm ];
}
